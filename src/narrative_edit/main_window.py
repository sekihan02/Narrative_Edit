...
from PySide6.QtGui import QAction, QActionGroup, QColor, QCloseEvent, QFont, QKeySequence, QPageLayout, QPageSize, QPainter, QPdfWriter, QShortcut
...
class MainWindow(QMainWindow):
    ...
    def open_file_dialog(self) -> None:
        path, _ = QFileDialog.getOpenFileName(self, self._t("ファイルを開く", "Open File"))
        if not path:
            return
        self.open_file(path)
    ...
    def export_submission_pdf_current_tab(self) -> bool:
        ...
        text = editor_tab.editor.toPlainText().replace("\r\n", "\n").replace("\r", "\n")
        page_units, total_pages = self._layout_submission_units(text, rows=40, cols=40)
        ...
        try:
            ...
            pass
        writer.setTitle(editor_tab.state.metadata.work_title or Path(path).stem)
        ...
        try:
            ...
            font = QFont(editor_tab.editor.font())
